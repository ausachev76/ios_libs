...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
  ...
  /*
   * IBA enhancement - remove stuff moved in precedence of completion block execution.
   * Found a bug(?), when calling setImageWithURL.. in completion block caused the downloader
   * being removed from the array (or other array contents changes) and removing by index here
   * caused entire app crash.
   */
    [cacheDelegates removeObjectAtIndex:idx];
    [cacheURLs removeObjectAtIndex:idx];
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
              ...
              NSDictionary *currentDownloadInfo = [[downloadInfo objectAtIndex:uidx] retain];
...
#if NS_BLOCKS_AVAILABLE
                if ([currentDownloadInfo objectForKey:@"success"])
                {
                    SDWebImageSuccessBlock success = [currentDownloadInfo objectForKey:@"success"];
                    success(image, NO);
                }
#endif
              [currentDownloadInfo release];
            }
            else
            {
...
#if NS_BLOCKS_AVAILABLE
                if ([currentDownloadInfo objectForKey:@"failure"])
                {
                    SDWebImageFailureBlock failure = [currentDownloadInfo objectForKey:@"failure"];
                    failure(nil);
                }
#endif
              [currentDownloadInfo release];
