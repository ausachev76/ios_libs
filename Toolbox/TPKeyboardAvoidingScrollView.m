...
@synthesize useFullFrameScrollAreaWithSmallContent = _useFullFrameScrollAreaWithSmallContent;
- (void)setup {
    _priorInsetSaved = NO;
    _useFullFrameScrollAreaWithSmallContent = YES;
...
-(void)setFrame:(CGRect)frame {
    ...
    // added by VAV: don't use full view frame as scroll area
    if ( _useFullFrameScrollAreaWithSmallContent )
    {
...
-(CGFloat)offsetToTopOfKeyboard:(UIView *)view withSpace:(CGFloat)space {
  ...
  if ( view.bounds.size.height < space ) {
    offset -= floor((space - view.bounds.size.height)) - 10.0f;
  }
...
- (void)adjustOffsetToTopOfKeyboard {
  ...
  CGPoint offsetToTopOfKeyboard = CGPointMake(0, [self offsetToTopOfKeyboard:[self findFirstResponderBeneathView:self] withSpace:visibleSpace]);
  
  [self setContentOffset:offsetToTopOfKeyboard animated:YES];
}
